...
export class DB2QueryRunner extends BaseQueryRunner implements QueryRunner {
  ...
  constructor(driver: DB2Driver, mode: "master" | "slave" = "master") {
    super();
    this.driver = driver;
  ...
  async startTransaction(
    ...
    databaseConnection.beginTransactionSync();
    this.isTransactionActive = true;

    // let db2IsolationLevel = "CS";
    // switch (isolationLevel.toString()) {
    //   case "READ UNCOMMITTED":
    //     db2IsolationLevel = "CS";
    //     break;

    //   case "SERIALIZABLE":
    //     db2IsolationLevel = "RR";
    //     break;

    //   case "REPEATABLE READ":
    //     db2IsolationLevel = "RS";
    //     break;

    //   case "READ UNCOMMITTED":
    //     db2IsolationLevel = "UR";
    //     break;
    // }
    ...
    // ) {
    //   throw new Error(
    //     `DB2 only supports SERIALIZABLE and READ COMMITTED isolation`
    //   );
    // }
  }
  ...
  async commitTransaction(): Promise<void> {
    ...
    const databaseConnection = await this.connect();
    databaseConnection.commitTransactionSync();
  ...
  stream(
    ...
    throw new Error(`Stream is not supported by DB2 driver.`);
  }
  ...
  async hasTable(tableOrName: Table | string): Promise<boolean> {
    ...
    const sql = `SELECT NAME FROM SYSIBM.SYSTABLES WHERE type = 'T' AND CREATOR='${this.driver.options.username!.toUpperCase()}' AND NAME='${tableName.toUpperCase()}'`;
    const result = await this.query(sql);
    console.log("hasTable check result", result);
  ...
  async hasColumn(
    ...
    const tableName =
      tableOrName instanceof Table ? tableOrName.name : tableOrName;
    const sql = `SELECT NAME FROM SYSIBM.SYSCOLUMNS WHERE TBCREATOR='${this.driver.options.username!.toUpperCase()}' AND TBNAME='${tableName.toUpperCase()} AND NAME='${columnName}'`;
    const result = await this.query(sql);
    return result.length ? true : false;
  }
  ...
  async createDatabase(database: string, ifNotExist?: boolean): Promise<void> {
    ...
    console.info("do not support db creation");
  }
  ...
  async createSchema(schemas: string, ifNotExist?: boolean): Promise<void> {
    throw new Error(`Schema create queries are not supported by DB2 driver.`);
  }
  ...
  async dropSchema(schemaPath: string, ifExist?: boolean): Promise<void> {
    throw new Error(`Schema drop queries are not supported by DB2 driver.`);
  }
  ...
  async createTable(
    ...
    console.info("why create table called ???? ", ifNotExist);
    console.trace();
  ...
  async renameTable(
    ...
    newTable.uniques.forEach(unique => {
      ...
      // build queries
      upQueries.push(
        `ALTER TABLE ${newTable.name} RENAME CONSTRAINT ${
          unique.name
        } TO ${newUniqueName}`
      );
      downQueries.push(
        `ALTER TABLE ${newTable.name} RENAME CONSTRAINT ${newUniqueName} TO ${
          unique.name
        }`
      );
  ...
  async addColumn(
    ...
    upQueries.push(
      `ALTER TABLE ${table.name} ADD ${this.buildCreateColumnSql(column)}`
    );
    ...
    const columnIndex = clonedTable.indices.find(
      index =>
        index.columnNames.length === 1 && index.columnNames[0] === column.name
    );
    if (columnIndex) {
    ...
    if (column.isUnique) {
      const uniqueConstraint = new TableUnique({
        name: this.connection.namingStrategy.uniqueConstraintName(table.name, [
          column.name
        ]),
  ...
  async renameColumn(
    ...
    if (!oldColumn)
      throw new Error(
        `Column "${oldTableColumnOrName}" was not found in the "${
          table.name
        }" table.`
      );
  ...
  async changeColumn(
    ...
    if (
      ...
      if (newColumn.name !== oldColumn.name) {
        // rename column
        upQueries.push(
          `ALTER TABLE ${table.name} RENAME COLUMN ${oldColumn.name} TO ${
            newColumn.name
          }`
        );
        downQueries.push(
          `ALTER TABLE ${table.name} RENAME COLUMN ${newColumn.name} TO ${
            oldColumn.name
          }`
        );
        ...
        clonedTable.findColumnUniques(oldColumn).forEach(unique => {
          ...
          downQueries.push(
            `ALTER TABLE ${table.name} RENAME CONSTRAINT ${newUniqueName} TO ${
              unique.name
            }`
          );
        ...
        clonedTable.columns[clonedTable.columns.indexOf(oldTableColumn!)].name =
          newColumn.name;
  ...
  async clearDatabase(): Promise<void> {
    console.info("clear database not implemented");
    return;
    /* await this.startTransaction();
    try {
      const dropTablesQuery = `SELECT 'DROP TABLE "' || NAME || '" CASCADE CONSTRAINTS' AS "query" FROM "USER_TABLES"`;
  ...
  protected async loadTables(tableNames: string[]): Promise<Table[]> {
    console.log("loadTables", tableNames, this.loadedTables);
    ...
    const tablesSql = `SELECT * FROM SYSIBM.SYSTABLES WHERE type='T' AND CREATOR='${this.driver.options.username!.toUpperCase()}' AND NAME IN (${tableNamesString.toUpperCase()})`.toString();
    const columnsSql = `SELECT * FROM SYSIBM.SYSCOLUMNS WHERE TBCREATOR='${this.driver.options.username!.toUpperCase()}' AND TBNAME IN (${tableNamesString.toUpperCase()})`.toString();

    const indicesSql = `SELECT IND.NAME, IND.UNIQUERULE, LISTAGG(COL.COLNAME,',') WITHIN GROUP (ORDER BY COL.COLNAME) AS COLUMN_NAMES
      FROM SYSIBM.SYSINDEXES IND
      INNER JOIN SYSIBM.SYSINDEXCOLUSE COL ON COL.INDNAME = IND.NAME
      LEFT JOIN SYSIBM.SYSTABCONST CON ON CON.NAME = IND.NAME
      WHERE IND.TBNAME IN (${tableNamesString.toUpperCase()})
      GROUP BY IND.NAME, IND.UNIQUERULE`.toString();

    const foreignKeysSql = `
    SELECT C.NAME as CONSTRAINT_NAME, C.TBNAME, COL.COLNAME, R.REFTBNAME AS REFERENCED_TBNAME, R.PKCOLNAMES AS REFERENCED_COLNAME, R.DELETERULE as ON_DELETE
    FROM SYSIBM.SYSTABCONST C
    INNER JOIN SYSIBM.SYSKEYCOLUSE COL ON COL.CONSTNAME = C.NAME
    INNER JOIN SYSIBM.SYSRELS R ON R.RELNAME = C.NAME
    WHERE C.TBNAME IN (${tableNamesString.toUpperCase()}) AND C.CONSTRAINTYP = 'F'
    `.toString();

    const constraintsSql = `
    SELECT C.NAME as CONSTRAINT_NAME, C.CONSTRAINTYP as CONSTRAINT_TYPE, COL.COLNAME, C.TBNAME
    FROM SYSIBM.SYSTABCONST C
    INNER JOIN SYSIBM.SYSKEYCOLUSE COL ON COL.CONSTNAME = C.NAME
    WHERE C.TBNAME IN (${tableNamesString.toUpperCase()}) AND C.CONSTRAINTYP IN ('C','U','P') AND C.TBCREATOR='${this.driver.options.username!.toUpperCase()}'
    `.toString();
    ...
    console.info("dbTables", dbTables.length);
    ...
    return dbTables.map(dbTable => {
      ...
      table.columns = dbColumns
        .filter(dbColumn => dbColumn["TBNAME"] === table.name)
        .map(dbColumn => {
          const columnConstraints = dbConstraints.filter(
            dbConstraint =>
              dbConstraint["TBNAME"] === table.name &&
              dbConstraint["COLNAME"] === dbColumn["NAME"]
          );
          ...
          tableColumn.type = dbColumn["COLTYPE"].trim().toLowerCase();
          ...
          if (tableColumn.type === "number" || tableColumn.type === "float") {
            if (
              ...
              tableColumn.precision = dbColumn["PRECISION"];
            if (
              dbColumn["SCALE"] !== null &&
          ...
          tableColumn.default =
            ...
            dbColumn["DEFAULT"] !== "NULL"
              ? (tableColumn.default = dbColumn["DEFAULT:"])
              : undefined;

          tableColumn.isNullable = dbColumn["NULLS"] === "Y";
          tableColumn.isUnique = isUnique;
          tableColumn.isPrimary = isPrimary;
          tableColumn.isGenerated = dbColumn["IDENTITY"] === "Y";
          ...
          tableColumn.type = tableColumn.type.trim();
          return tableColumn;
        });
      ...
      table.uniques = tableUniqueConstraints.map(constraint => {
        ...
        return new TableUnique({
          ...
          columnNames: uniques.map(u => u["COLNAME"])
        });
      });
      ...
      const tableCheckConstraints = OrmUtils.uniq(
        ...
        dbConstraint => dbConstraint["COLNAME"]
      );
      ...
      table.checks = tableCheckConstraints.map(constraint => {
        ...
        return new TableCheck({
          name: constraint["CONSTRAINT_NAME"],
          columnNames: checks.map(c => c["COLNAME"]),
      ...
      const tableForeignKeyConstraints = OrmUtils.uniq(
        dbForeignKeys.filter(dbForeignKey => {
          return dbForeignKey["TBNAME"] === table.name;
        }),
      ...
      table.foreignKeys = tableForeignKeyConstraints.map(dbForeignKey => {
        ...
        return new TableForeignKey({
          ...
          columnNames: foreignKeys.map(dbFk => dbFk["COLNAME"]),
          referencedTableName: dbForeignKey["REFERENCED_TBNAME"],
          referencedColumnNames: foreignKeys.map(
            dbFk => dbFk["REFERENCED_COLNAME"]
          ),
          onDelete:
            dbForeignKey["ON_DELETE"] === "NO ACTION"
              ? undefined
      ...
      table.indices = dbIndices
        .filter(dbIndex => dbIndex["TBNAME"] === table.name)
        .map(dbIndex => {
  ...
  protected createTableSql(table: Table, createForeignKeys?: boolean): string {
      ...
      const foreignKeysSql = table.foreignKeys
        .map(fk => {
          ...
          let constraint = `CONSTRAINT ${
            fk.name
          } FOREIGN KEY (${columnNames}) REFERENCES ${
    ...
    if (primaryColumns.length > 0) {
      ...
      sql += ``;
      console.log(
        `, CONSTRAINT ${primaryKeyName} PRIMARY KEY (${columnNames})`
      );
    }
  ...
  protected createIndexSql(table: Table, index: TableIndex): string {
    const columns = index.columnNames
      .map(columnName => `${columnName}`)
      .join(", ");
    return `CREATE ${index.isUnique ? "UNIQUE " : ""}INDEX ${index.name} ON ${
      table.name
    }(${columns})`;
  }
  ...
  protected createUniqueConstraintSql(
    ...
    const columnNames = uniqueConstraint.columnNames
      .map(column => `` + column + ``)
      .join(", ");
    return `ALTER TABLE ${table.name} ADD CONSTRAINT ${
      uniqueConstraint.name
    } UNIQUE (${columnNames})`;
  }
  ...
  protected dropUniqueConstraintSql(
    ...
    const uniqueName =
      uniqueOrName instanceof TableUnique ? uniqueOrName.name : uniqueOrName;
  ...
  protected buildCreateColumnSql(column: TableColumn) {
    ...
    if (
      ...
      c += ` PRIMARY KEY GENERATED ALWAYS AS IDENTITY(START WITH 1 INCREMENT BY 1)`;
